...
NOTE: NO EXPLAINATIONS. JUST PURE WORKFLOW. NO SUGGESTIONS
...
def extract_events_from_annotations(annotations):
    events = [annotation["text"] for annotation in annotations]
    return "-".join(events)
...
def main():
    ...
    uploaded_file = st.file_uploader("Upload Annotated JSON File", type="json")
    if uploaded_file is not None:
        data = json.load(uploaded_file)
        annotations = data.get("annotations", [])
        user_input = extract_events_from_annotations(annotations)
        st.text(f"Extracted events: {user_input}")
